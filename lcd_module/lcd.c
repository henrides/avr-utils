...
void lcd_set_cursor_pos(char addr)
{
	LCD_CONTROL_PORT &= ~(1<<LCD_CONTROL_RS);
	send_data(LCD_CMD_DDRAM_ADDRESS | addr);
}

void lcd_set_cursor_display(char flags)
{
	LCD_CONTROL_PORT &= ~(1<<LCD_CONTROL_RS);
	send_data(LCD_CMD_DISPLAY | LCD_FLAG_DISPLAY_D | flags);
}
...
void lcd_init(char display_flags)
{
	...
	lcd_set_cursor_display(LCD_FLAG_DISPLAY_D | display_flags);
